...
export const WhyKuodia: React.FC = () => {
  ...
  const items = [
    ...
    {
      icon: <TrendingDown className="h-5 w-5 text-brand-green" />,
  ...
  return (
    <div className="bg-white rounded-3xl p-6 sm:p-8 shadow-xl shadow-slate-900/5 w-full">
      {/* Title */}
      <h3 className="font-display font-black text-lg text-brand-navy uppercase tracking-wider mb-6 pb-4 border-b border-slate-100">
        {t("why_farm_title")}
      </h3>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-6">
        {items.map((item, idx) => (
          <div key={idx} className="flex items-start space-x-3.5">
            ...
            <div className="flex-shrink-0 h-10 w-10 rounded-full border border-brand-green/20 bg-brand-green-bg/30 flex items-center justify-center">
              {item.icon}
            </div>
            {/* Content */}
            <div className="flex flex-col space-y-1">
              <span className="text-xs sm:text-sm font-display font-black text-brand-navy leading-snug">
                {item.title}
              </span>
              <span className="text-[10px] sm:text-xs font-sans font-semibold text-slate-600 leading-normal">
                {item.desc}
              </span>
            </div>
          </div>
